...
bool solve(vector<vector<string>> &board, string word)
{
    ...
    if (!(board[0].size() < word.length()))
    {
        for (int j = 0; j < board.size(); j++)
        {
            ...
            {
                if (board[0].size() - i < word.length())
                    break;
                string s;
                s.push_back(word[0]);
                if (s == board[j][i])
                {
                    if (row_check(board, word, j, i))
                        return true;
                }
            }
        }
